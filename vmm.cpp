...
int main()
{
    ...
    while(fin.peek()!=EOF)
    {
        ...
        page_no=logicaladdress/int(PAGE_SIZE); //get page number from logical address
        offset=logicaladdress % int(PAGE_SIZE); //get offset number from logical address
	...
	if(hit!=1)
		...
		//replace page in tlb using fifo
		tlb[qp][0]=page_no;
		tlb[qp][1]=i;
		qp++;
		qp=qp%(TLB_SIZE-1);	
